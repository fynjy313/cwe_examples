...
public class HibernateProductController {
    ...
    @GetMapping("/products_byName_HQLSafety/{name}")
    public List<Product> findByName_HQL(@PathVariable String name) {
        return repository.findByName_HQL_safety(name);
    }

    //%27%20or%20%271%27=%271
    @GetMapping("/products_byName_HQLUnsafe/{name}")
    public List<Product> findByName_HQL_unsafe(@PathVariable String name) {
        return repository.findByName_HQL_unsafe(name);
    }

    @GetMapping("/products_byName_HQL_Session_unsafe/{name}")
    ...
    @GetMapping("/products_byName_HQL_Session_safety/{name}")
